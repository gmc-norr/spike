...
fn identify_haplotype_reads(
    ...
    label: &str,
    rng: &mut StdRng,
) -> Result<(HashSet<String>, HashSet<String>, HashMap<u64, u8>)> {
    ...
    let (het_snps, read_alleles) = if let Some(gvcf) = gvcf_path {
        ...
        if snps.is_empty() {
            ...
            pileup_and_collect(bam_path, chrom, region_start, region_end, min_mapq)?
        } else {
            // gVCF gave us het SNPs; we still need one BAM pass to classify reads.
            (snps, None)
        }
    } else {
        pileup_and_collect(bam_path, chrom, region_start, region_end, min_mapq)?
    ...
    // Classify reads: use pre-collected alleles if available, otherwise BAM pass.
...
pub fn identify_deleted_haplotype_reads(
    ...
    let (target_set, classified_set, _variants) = identify_haplotype_reads(
        bam_path, chrom, del_start, del_end, min_mapq, gvcf_path, "LOH-DEL", rng,
    )?;
...
pub fn identify_duplicated_haplotype_reads(
    ...
    identify_haplotype_reads(
        bam_path, chrom, dup_start, dup_end, min_mapq, gvcf_path, "AI-DUP", rng,
    )
}
...
fn pileup_and_collect(
    ...
    region_end: u64,
    min_mapq: u8,
) -> Result<(Vec<HetSnp>, Option<HashMap<String, Vec<(u64, u8)>>>)> {
    // allele_counts[pos] = [A_count, C_count, G_count, T_count]
    let mut allele_counts: HashMap<u64, [u32; 4]> = HashMap::new();
    // read_alleles[name] = Vec<(pos, base)> — all observed bases per read name
    ...
    let start_pos =
        noodles::core::Position::try_from(1_usize.max(region_start as usize + 1)).unwrap();
    let end_pos = noodles::core::Position::try_from(region_end as usize).unwrap();
    ...
    for rec_result in query {
        ...
        // Walk CIGAR to map sequence bases to reference positions.
    ...
    // Find het SNP positions: 2 alleles each at 20-80%, depth >= 10.
    let mut het_snps = Vec::new();
    ...
    for (&pos, counts) in &allele_counts {
        ...
        if sorted.len() >= 2 {
            ...
            if f1 >= 0.2 && f1 <= 0.8 && f2 >= 0.2 && f2 <= 0.8 {
                het_snps.push(HetSnp {
                    pos,
                    allele1: sorted[0].0,
                    allele2: sorted[1].0,
...
fn classify_from_collected(
    ...
    log::info!(
        ...
        classified_set.len(),
        target_set.len(),
        classified_set.len() as u32 - target_set.len() as u32 - n_ambiguous,
        n_ambiguous,
    );
...
fn classify_reads_by_haplotype(
    ...
    // Per read-name: (target_allele_count, other_allele_count)
                    ...
                    for i in 0..len {
                        ...
                        if let Some(&tgt_allele) = target_allele.get(&rp) {
                            if let Some(&base) = seq.get(seq_pos + i) {
                                let base = base.to_ascii_uppercase();
                                if base == tgt_allele {
                                    scores.0 += 1; // matches target allele
                                } else if base != b'N' {
                                    scores.1 += 1; // matches other allele
                                }
                            }
                        }
                    }
    ...
    // Build target set and classified set.
    let mut target_set = HashSet::new();
    let mut classified_set = HashSet::new();
    let mut n_ambiguous = 0u32;
    ...
    for (name, (target_count, other_count)) in &read_scores {
        if *target_count == 0 && *other_count == 0 {
            continue; // no het SNP overlap — unclassifiable
        }
        classified_set.insert(name.clone());
        ...
        if *target_count > *other_count {
            ...
            // Tie: don't include (slightly biases toward under-selection,
            // but ties are rare with multiple het SNPs per read).
            n_ambiguous += 1;
        }
    }
